...
<main class="page-content">
    <div class="card border shadow-none">
        ...
        <!-- Konten -->
        <?= form_open_multipart('saldo/saldodriver/'.$this->session->flashdata('id_user')); ?>
            <input type="hidden" name="id" value="<?= $this->session->flashdata('id_user') ?>">
            <input type="hidden" name="saldolama" value="<?= $pelanggan['saldo'] ?>">
            <div class="row mb-3">
                <label for="inputEnterYourName" class="col-sm-2 col-form-label">Akun</label>
                <div class="col-sm-9">
                    <input type="text" class="form-control" id="title" name="atasnama" value="<?= $pelanggan['fullnama'] ?>" disabled>
                </div>
            </div>
            <div class="row mb-3">
                <label for="inputEnterYourName" class="col-sm-2 col-form-label">Saldo Saat Ini</label>
                <div class="col-sm-9">
                    <input type="text" class="form-control" id="title" value="<?= $currency['app_currency'] ?><?= rupiah($pelanggan['saldo']) ?>" disabled>
                </div>
            </div>
            <div class="row mb-3">
