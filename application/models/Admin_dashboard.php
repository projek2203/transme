...
class Admin_dashboard extends CI_model
{
  ...
  {
    $this->db->select('transaksi.*,' . 'driver.nama_driver,' . 'driver.reg_id,' . 'pelanggan.fullnama,' . 'pelanggan.token,' . 'history_transaksi.*,' . 'status_transaksi.*,' . 'fitur.fitur,'. 'fitur.icon');
    ...
    $this->db->order_by('history_transaksi.id_transaksi', 'DESC');
    return $this->db->get()->result_array();
  }
  public function DashboardTotalTransaksi()
  {
  ...
  {
    $this->db->select('transaksi.*,' . 'driver.nama_driver,' . 'driver.reg_id,' . 'pelanggan.fullnama,' . 'pelanggan.token,' . 'history_transaksi.*,' . 'status_transaksi.*,' . 'fitur.fitur,fitur.home,fitur.icon');
    $this->db->from('transaksi');
    $this->db->join('history_transaksi', 'transaksi.id = history_transaksi.id_transaksi', 'left');
    $this->db->join('status_transaksi', 'history_transaksi.status = status_transaksi.id', 'left');
    $this->db->join('driver', 'transaksi.id_driver = driver.id', 'left');
  ...
  function aksesperizinan($grup)
  {
    $this->db->select('akses.kode','akses.grup');
    $this->db->from('akses');
  ...
  {
    $this->db->select('SUM(saldo)as totalsaldo');
  ...
  public function gettransaksibyid($id)
  {
    ...
    $this->db->select('transaksi.*,transaksi.id_driver as driverid');
  ...
  public function pendapatan_tahunan(){
    $day = date('d');
    $month = date('m');
    $year = date('Y');
    $this->db->select('*');
  ...
  public function pendapatan_aplikasi(){
    $this->db->select('*');
    $this->db->from('komisi');
    $cek = $this->db->get();
    $countdata = 0;
    ...
    if ($cek->num_rows() > 0) {
        foreach ($cek->result() as $row) {
            $countdata += $row->komisi;
        }
        $hasil = $countdata;
    }
    return $hasil;
  }
  public function notiftodriver($id,$token,$serverkey)
  {
