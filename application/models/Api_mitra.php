...
class Api_mitra extends CI_model
{
    ...
    public function cek_data_merchant($condition)
    {
    ...
    public function data_merchant($idmerchant)
    {
        $this->db->select('*');
        $this->db->from('merchant');
        $this->db->join('category_merchant', 'merchant.category_merchant = category_merchant.id_kategori_merchant');
    ...
    public function signup($data_signup, $data_merchant, $data_berkas)
    {
       ...
       // $inserid = $this->db->insert_id();
        /*$datasignup = array(
            ...
            'partner' => '0',
            'id_merchant' => $data_merchant['id_merchant'],
            'status_mitra' => '0'
        );*/
        $datasignup = array(
            'id_mitra' => $data_signup['id_mitra'],
            'nama_mitra' => $data_signup['nama_mitra'],
            'jenis_identitas_mitra' => 'KTP',
            'nomor_identitas_mitra' => '123456',
            'alamat_mitra' => $data_signup['alamat_mitra'],
    ...
    //------------------ List Banner -----------------------------------------
    function list_slider($fitur)
    {
        $this->db->select('*');
        $this->db->from('promosi');
        $this->db->where('promosi.fitur_promosi', $fitur);
    ...
    public function total_history_daily($day, $idmerchant)
    {
        ...
        $this->db->select('SUM(transaksi_detail_merchant.total_biaya) daily,COUNT(transaksi_detail_merchant.id_merchant) as counter');
        $this->db->from('transaksi_detail_merchant');
    ...
    public function addkategori($nama, $status, $id,$foto)
    {
    ...
    //------------------------ Chat -------------------------------------------
    public function datainbox($idmerchant)
    {
        $this->db->select('*','driver.*','history_transaksi.status');
        $this->db->from('chat_merchant');
        $this->db->join('history_transaksi', 'chat_merchant.idtrans = history_transaksi.id_transaksi', 'left');
        ...
        $this->db->where('idmerc', $idmerchant);
        $this->db->order_by('chat_merchant.id DESC');
        $this->db->limit('1');
        return $this->db->get()->result_array();
    }
    ...
    {
        $this->db->where('id_merchant', $idpelanggan);
        $this->db->update('merchant', $data);
        return true;
    }
