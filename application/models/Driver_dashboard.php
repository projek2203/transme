...
class Driver_dashboard extends CI_model
{
   ...
   function cut_driver_saldo_by_order($data)
    {
        ...
        if ($data['pakai_wallet'] == 1) {
            ...
            $kalkulasi = $data['biaya_akhir'] - $jasaapp;
    ...
    public function getAlldriver()
    {
        ...
        $this->db->select('berkas_driver.foto_ktp,berkas_driver.foto_sim,berkas_driver.id_sim');
        $this->db->select('saldo.saldo,saldo.saldo as totalsaldo');
        $this->db->select('driver.status');
        $this->db->select('driver.*');
    ...
    public function transaksisukses($id)
    {
        $this->db->select('status_transaksi.*');
        $this->db->select('history_transaksi.*');
        $this->db->select('fitur.*');
    ...
    public function ubahstatusnewreg($id,$linkuser,$kota)
    {
        $this->db->set('status', 1);
...
function get_driver_location($idDriver)
  ...
  function my_token($data_l)
    ...
    function totalpendapatanmingguan($id)
    {
        ...
        $this->db->select('*');
        $this->db->from('wallet');
        $this->db->where('waktu BETWEEN DATE_SUB(NOW(), INTERVAL 15 DAY) AND NOW()');
        $this->db->where('id_user', $id);
        $this->db->where("(type = 'Order-' OR type = 'Order+')", NULL, FALSE);
        $this->db->where('status', '1');
        $cek = $this->db->get();
...
<!--[if (mso)|(IE)]></td></tr></table><![endif]-->
    ...
    {
        $this->db->select('upgrade.iddriver,upgrade.tanggal as tgl,upgrade.status as setatus,driver.*');
        $this->db->join('driver', 'upgrade.iddriver = driver.id');
        return  $this->db->get('upgrade')->result_array();
    }
    public function statusupgrade($data)
    {
        $this->db->set('prioritas', 1);
        $this->db->where('id', $data['id']);
        return $this->db->update('driver', $data);
    }
    public function approvedupg($data)
    {
        $this->db->set('status', $data['status']);
        $this->db->where('iddriver', $data['id']);
        return $this->db->update('upgrade', $data);
    }
    function cekpelangganbytrans($idtrans)
    {
        $this->db->select('*');
        $this->db->from('transaksi');
        $this->db->join('pelanggan', 'transaksi.id_pelanggan = pelanggan.id');
        $this->db->where('transaksi.id', $idtrans);
        $trans = $this->db->get();
        return $trans;
    }
    function cekdriverbytrans($idtrans)
    {
        $this->db->select('*');
        $this->db->from('transaksi');
        $this->db->join('driver', 'transaksi.id_driver = driver.id');
        $this->db->where('transaksi.id', $idtrans);
        $trans = $this->db->get();
        return $trans;
    }
    function cekstatusbytrans($idtrans)
    {
        $this->db->select('*');
        $this->db->from('transaksi');
        $this->db->join('history_transaksi', 'transaksi.id = history_transaksi.id_transaksi', 'left');
    ...
    function cektrxmerchantbyid($idtrans)
    {
        $this->db->select('*');
        $this->db->from('transaksi_detail_merchant');
        $this->db->join('merchant', 'transaksi_detail_merchant.id_merchant = merchant.id_merchant');
        $this->db->where('id_transaksi', $idtrans);
        $trans = $this->db->get();
        return $trans;
    }
    function cektransaksibyid($idtrans)
    {
        $this->db->select('*');
        $this->db->from('transaksi');
        $this->db->join('transaksi_detail_merchant', 'transaksi.id = transaksi_detail_merchant.id_transaksi', 'left');
